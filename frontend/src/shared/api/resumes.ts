...
export type ResumeAdaptationsSummaryResponse = { resumeId: string; count: number };
...
export async function getResumeAdaptationsSummary(resumeId: string, accessToken: string) {
  const response = await fetch(`${getApiUrl()}/api/resumes/${resumeId}/adaptations/summary`, {
    headers: createAuthHeaders(accessToken),
  });
  return parseApiResponse<ResumeAdaptationsSummaryResponse>(
    response,
    'Failed to fetch resume adaptations summary'
  );
}
...
export async function updateResumeText(params: {
  ...
  contacts: import('@/src/features/resume-editor/model/types').ContactDraft;
