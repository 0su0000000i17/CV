...
export function handleResumeUpload(
  ...
  resumeUpload.single("resume")(req, res, (error: unknown) => {
    ...
    // The Content-Type header is client-supplied and easy to spoof, so we
    // also confirm the file actually starts with the PDF magic bytes before
    // it reaches the parsing/rendering pipeline.
