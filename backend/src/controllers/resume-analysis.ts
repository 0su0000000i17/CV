...
import {
  ...
  mapAnalysisRow,
} from "../resume-analysis/presenter.js";
import {
  getStringParam,
  sendError,
  sendServerError,
...
export async function getLatestResumeAnalysis(req: Request, res: Response) {
  try {
    ...
    const resume = await findResumeOwnerRecord({
      userId: user.id,
      resumeId,
    });
...
export async function analyzeResumePreview(req: Request, res: Response) {
  ...
  try {
    ...
    if (currentResumeCachedAnalysis) {
      console.info(
        `[resumeAnalysis] Cache hit for current resume ${resume.id}. AI call skipped.`
      );
      ...
      return res.json(
        mapAnalysisRow(currentResumeCachedAnalysis, {
          cached: true,
    ...
    const userReusableCache = await findUserReusableCache({
      userId: user.id,
      cacheKey,
    });
    ...
    if (userReusableCache) {
      console.info(
        `[resumeAnalysis] User cache hit for resume ${resume.id}. AI call skipped.`
      );
      ...
      return res.json(
        mapAnalysisRow(cachedAnalysisForCurrentResume, {
          cached: true,
    ...
    await upsertResumeAnalysisCache({
      userId: user.id,
      cacheKey,
      analysisRow: savedAnalysis,
    });
