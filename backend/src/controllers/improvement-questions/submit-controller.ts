...
export async function submitImprovementAnswersController(req: Request, res: Response) {
  try {
    ...
    if (!resumeId || !sessionId) {
      return sendError(res, 400, "Invalid improvement session id");
    }
    ...
    const { data, error } = await supabaseAdmin
      .from("resume_improvement_sessions")
      .update(updatePayload)
      .eq("id", sessionId)
      .eq("resume_id", resumeId)
      .eq("user_id", user.id)
      .select(IMPROVEMENT_SESSION_SELECT)
      .maybeSingle();
    if (error) return sendServerError(res, "Failed to save answers", error);
    if (!data) return sendError(res, 404, "Improvement session not found");
    return res.json({ status: "ok", session: data as ImprovementSessionRow });
  } catch (error) {
    return sendServerError(res, "Unexpected error saving improvement answers", error);
  }
}
