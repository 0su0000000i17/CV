...
function createFileName(...candidates: Array<string | null | undefined>) {
  const baseName = candidates
    .map((value) =>
      String(value || "")
        .replace(/[\\/:*?"<>|]+/g, " ")
        .replace(/\s+/g, " ")
  ...
  return encodeURIComponent(`${baseName || "resume"}.pdf`);
}
...
export async function exportClassicResumeController(req: Request, res: Response) {
  try {
    ...
    const source = await getResumeExportSource({ userId: user.id, resumeId });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename*=UTF-8''${createFileName(parsedBody.data.contacts.fullName, sourceTitle)}`
    );
