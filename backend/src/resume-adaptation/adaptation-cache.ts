...
const CACHE_VERSION = "adaptation-cache-v12";
const USER_PROMPT_VERSION = "adaptation-user-prompt-v6";
...
export function createAdaptationCacheMetadata(params: {
  ...
  // Confirmed clarifying-question answers change the generation input, so two
  // runs with different answers must never share a cache entry.
  const resumeHash = createSha256Hash(
    [params.resumePayload, ...(params.confirmedFacts || [])].join("\n__FACT__\n")
  );
