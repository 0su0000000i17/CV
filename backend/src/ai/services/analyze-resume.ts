...
function createBaseMessages(resumeMarkdown: string): AiMessage[] {
  return [
    {
      role: "system",
    ...
    {
      role: "user",
      content: createAnalyzeResumeUserPrompt(resumeMarkdown),
    },
  ];
}
...
function createRetryMessages(
  ...
  return [
    ...
    {
      role: "user",
      content: createStrictRetryPrompt(resumeMarkdown, errorMessage),
    },
  ];
}
...
export async function analyzeResume(params: AnalyzeResumeParams) {
  ...
  try {
    parsedResult = await generateAndParseAnalysis(
      createBaseMessages(params.resumeMarkdown),
    ...
    parsedResult = await generateAndParseAnalysis(
      createRetryMessages(params.resumeMarkdown, errorMessage),
      RETRY_MAX_TOKENS
    );
  }
